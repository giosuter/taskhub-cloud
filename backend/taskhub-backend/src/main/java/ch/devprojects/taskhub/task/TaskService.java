...
public class TaskService {
	...
	public TaskService(TaskRepository repo, TaskMapper mapper) {
		this.repo = repo;
		this.mapper = mapper; // injected @Component TaskMapperImpl
	}

	@Transactional
	public TaskResponse create(String ownerId, TaskRequest req) {
		var t = mapper.toEntity(req);
	...
	public List<TaskResponse> list(String ownerId) {
		return repo.findByOwnerIdOrderByCreatedAtDesc(ownerId).stream().map(mapper::toResponse).toList();
	}
